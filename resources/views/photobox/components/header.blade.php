...
<div id="camera-settings-panel" class="hidden fixed top-0 left-0 right-0 z-50 bg-slate-800/95 backdrop-blur-sm border-b border-slate-700 px-6 py-4">
    <div class="max-w-4xl mx-auto flex items-center justify-between">
        ...
        <button onclick="if(typeof toggleCameraSettings === 'function') { toggleCameraSettings(); } else { console.error('toggleCameraSettings not available!'); alert('Camera settings function not available. Please refresh the page.'); }" 
...
<header class="bg-white/10 backdrop-blur-md border-b border-white/20 p-4">
    <div class="flex items-center justify-between">
        <div class="flex items-center space-x-4">
            <div class="w-12 h-12 rounded-xl overflow-hidden bg-white/10 flex items-center justify-center shadow-lg border border-white/20">
                <img src="/logo-fotoku-landscape.png" alt="FOTOQU Logo" class="object-contain w-11 h-11" onerror="this.style.display='none'">
                <i class="fas fa-camera text-white text-xl" style="display:none"></i>
            </div>
            <div>
                <h1 class="text-2xl font-bold text-white">FOTOQU Photobooth</h1>
        ...
        <div class="flex items-center space-x-4">
            ...
            <button id="camera-settings-toggle" onclick="if(typeof toggleCameraSettings === 'function') { toggleCameraSettings(); } else { console.error('toggleCameraSettings not available!'); alert('Camera settings function not available. Please refresh the page.'); }" 
