    ...
    <div class="glassmorphism rounded-3xl p-8 text-white">
                ...
                <div class="space-y-4">
                    <div class="bg-white/10 rounded-2xl p-6">
                        ...
                        <ul class="text-left space-y-2 text-white/80">
                            <li>✨ System akan mengambil {{ config('fotoku.total_photos', 3) }} foto otomatis</li>
                            <li>🎯 Anda akan memilih {{ config('fotoku.frame_templates.fotostrip.user_selection', 3) }} foto terbaik</li>
                            <li>📸 Jeda {{ config('fotoku.photo_interval_seconds', 3) }} detik antar foto</li>
        ...
        <div x-show="currentStep === 'photo-session'" class="text-center">
            <div class="mb-8">
                ...
                <p class="text-xl text-white/80 mb-8">
                    Foto <span x-text="currentPhotoNumber"></span> dari {{ config('fotoku.total_photos', 3) }}
                </p>
                ...
                <div class="w-full bg-white/20 rounded-full h-4 mb-8">
                    <div class="bg-gradient-to-r from-green-500 to-emerald-600 h-4 rounded-full transition-all duration-500"
                         :style="`width: ${(currentPhotoNumber / {{ config('fotoku.total_photos', 3) }}) * 100}%`"></div>
                </div>
        ...
        <div x-show="currentStep === 'selection'" class="text-center">
            <div class="mb-8">
                ...
                <p class="text-xl text-white/80 mb-8">
                    Pilih {{ config('fotoku.frame_templates.fotostrip.user_selection', 3) }} foto terbaik untuk frame Anda (dari {{ config('fotoku.total_photos', 3) }} foto yang diambil)
                </p>
                
                <!-- Photo Grid -->
                <div :class="`photo-grid grid-{{ config('fotoku.total_photos', 3) }}`">
            ...
            <div class="mb-8">
                ...
                <div class="bg-white/10 rounded-2xl p-6 mb-8">
                    ...
                    <ul class="space-y-2 text-white/80">
                        ...
                        <li>🖨️ Frame bisa langsung dicetak ukuran 4x6 (2 strip 2x3)</li>
...
function photoboxApp() {
    return {
        ...
        init() {
            ...
            // Simulate some captured photos for demo (5 photos max)
            this.capturedPhotos = Array.from({length: {{ config('fotoku.total_photos', 3) }}}, (_, i) => ({
                id: i + 1,
        ...
        get countdownStyle() {
            const progress = ((3 - this.countdown) / 3) * 100;
        ...
        simulatePhotoCapture() {
            const totalPhotos = {{ config('fotoku.total_photos', 3) }};
            const capturePhoto = () => {
        ...
        confirmSelection() {
            if (this.selectedPhotos.length === {{ config('fotoku.frame_templates.fotostrip.user_selection', 3) }}) {
                this.processFrame();
            }
        },
