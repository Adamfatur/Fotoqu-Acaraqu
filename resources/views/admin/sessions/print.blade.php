...
<head>
    ...
    @php
        ...
        if (!in_array($paper, ['100x148mm','4x6in'])) { $paper = '100x148mm'; }
        if ($paper === '100x148mm') {
            $PAGE_W = '100mm'; $PAGE_H = '148mm';
            $LABEL = '100×148 mm (Postcard)';
            // 300 DPI physical pixels (for fallbacks/docs)
            $PX_W = 1181; $PX_H = 1748;
        } else { // 4x6in
            $PAGE_W = '4in'; $PAGE_H = '6in';
            $LABEL = '4×6 in';
            $PX_W = 1200; $PX_H = 1800;
        }
        ...
        * {
        ...
        @media print {
            ...
            .frame-image {
                ...
                max-width: {{ $PAGE_W }} !important;
                max-height: {{ $PAGE_H }} !important;
                min-width: {{ $PAGE_W }} !important;
                min-height: {{ $PAGE_H }} !important;
                border: none !important;
                box-shadow: none !important;
                margin: 0 !important;
                padding: 0 !important;
                object-fit: cover !important; /* Full bleed - cover entire paper */
            ...
            @page {
                size: {{ $PAGE_W }} {{ $PAGE_H }}; /* Exact target */
                margin: 0 !important; /* Full bleed - no margins */
                padding: 0 !important;
                border: none;
            }
    ...
    <div class="print-container">
        <div class="print-info">
            ...
            <div style="margin-top:10px; display:flex; gap:8px; justify-content:center; flex-wrap:wrap;">
                <a class="btn btn-outline {{ $paper==='100x148mm' ? 'active' : '' }}" href="?paper=100x148mm{{ $autoPrint ? '&autoprint=1' : '' }}">100×148 mm</a>
                <a class="btn btn-outline {{ $paper==='4x6in' ? 'active' : '' }}" href="?paper=4x6in{{ $autoPrint ? '&autoprint=1' : '' }}">4×6 in</a>
            </div>
            <div style="margin-top:10px; display:flex; gap:8px; align-items:center; justify-content:center; flex-wrap:wrap;">
                ...
                <div style="display:flex; gap:6px; align-items:center;">
                    <input id="offsetYInput" type="number" step="0.1" min="-3" max="3" value="{{ $offsetY }}" style="width:80px; padding:8px; border:1px solid #d1d5db; border-radius:6px;">
        ...
        <img src="{{ route('photobox.serve-frame', ['frame' => $session->frame->id]) }}" 
             alt="Frame {{ $session->session_code }}" 
             class="frame-image">
        
        <div class="print-buttons">
            <button onclick="printWithDialog()" class="btn btn-print">🖨️ Print (Muncul Dialog)</button>
            <button onclick="kioskAutoPrint()" class="btn btn-outline">⚡ Print Langsung (Kiosk)</button>
