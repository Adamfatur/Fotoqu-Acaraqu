...
<script>
    ...
    console.log('💻 Camera init mode:', IS_DEV ? 'Development (flexible resolution)' : 'Production (Full HD enforced)');
    ...
    (function () {
        ...
        if (window.requestIdleCallback) {
            requestIdleCallback(() => {
                console.log('🚀 Camera performance optimizations loaded');
            });
        }
    })();
    ...
    async function initializeCamera() {
        try {
            ...
            try {
                ...
                console.log('📊 Browser supports these constraints:', supportedConstraints);

                // Check if specific width/height constraints are supported
                if (supportedConstraints.width && supportedConstraints.height) {
                    console.log('✅ Browser supports width/height constraints');
                } else {
                    console.error('⚠️ Browser might NOT support width/height constraints!');
                }

                // If Firefox, adjust constraints approach
                if (isFirefox) {
                    console.log('🦊 Firefox detected - might need to handle constraints differently!');
                }
            } catch (e) {
            ...
            console.log('📷 Camera base constraints:', cameraConstraints === true ? 'auto (true)' : cameraConstraints);

            // Attempt to initialize camera with appropriate constraints
            try {
                console.log(IS_DEV ? '📷 DEV: starting camera with flexible constraints' : '📷 PROD: enforcing Full HD 1920x1080');
                ...
                if (hasLogitechBrio && !IS_DEV) {
                    console.log('🎥 LOGITECH BRIO DETECTED! Menggunakan konfigurasi khusus Logitech');
                ...
                console.log('⚙️ Final constraints:', cameraConstraints === true ? 'auto (true)' : JSON.stringify(cameraConstraints, null, 2));
                ...
                if (videoTrack) {
                    ...
                    if (statusEl) {
                        if (!IS_DEV && settings.width >= 1920) {
                            statusEl.innerHTML = '<div class="w-2 h-2 bg-green-400 rounded-full animate-pulse mr-2"></div>✅ Full HD - ' + settings.width + 'x' + settings.height + '@' + settings.frameRate + 'fps';
                        } else if (settings.width >= 1280) {
                            const label = IS_DEV ? '✅ HD (Dev)' : '✅ HD';
                            ...
                            const label = IS_DEV ? '⚠️ Low quality (Dev)' : '❌ LOW QUALITY';
                            ...
                            if (!IS_DEV) {
                                console.error('❌ UNACCEPTABLE resolution:', settings.width + 'x' + settings.height);
                            }
                        }
                    }
                }
                ...
                if (IS_DEV) {
                    try {
                        console.log('🔄 DEV fallback: trying video: true');
                        ...
                        console.warn('⚠️ DEV permissive fallback also failed:', devErr);
                    }
                }
                throw error;
            }
        ...
        await new Promise(resolve => {
            requestAnimationFrame(() => {
                ...
                if (captureWidth < 1920 || captureHeight < 1080) {
                    console.error('❌ RESOLUSI TIDAK SESUAI!', `${captureWidth}x${captureHeight} bukan 1920x1080`);
                    console.error('📺 Video stream hanya:', `${video.videoWidth}x${video.videoHeight}`);
                } else {
                    console.log('✅ FULL HD PHOTO CAPTURE:', `${captureWidth}x${captureHeight}`);
                    console.log('📺 Video stream size:', `${video.videoWidth}x${video.videoHeight}`);
                }
                ...
                canvas.toBlob((blob) => {
                    try {
                        ...
                        console.log('📸 Photo captured successfully (non-blocking):', {
    ...
    // Background uploader: delegates to UploadManager
    async function uploadCapturedPhoto(localPhoto, blob) {
        try {
            // Ensure we have a session ID
            const sessionId = (window.currentSession && window.currentSession.id) ? window.currentSession.id : 'temp-session';

            // Enrich local photo object for storage
            localPhoto.sessionId = sessionId;
            localPhoto.blob = blob; // Store blob for retry
            localPhoto.timestamp = Date.now();

            // Add to robust upload manager
            if (window.uploadManager) {
                await window.uploadManager.addToQueue(localPhoto);
            } else {
                console.error('UploadManager not found! Fallback to direct upload.');
                // Fallback implementation if needed, or just error
            }
        } catch (e) {
    ...
    document.addEventListener('DOMContentLoaded', async () => {
        if (!window.uploadManager || !window.currentSession) return;

        try {
            // First, clean up any data from previous/other sessions to prevent data leakage
            if (window.photoStorage) {
                await window.photoStorage.cleanupOtherSessions(window.currentSession.id);
            }

            const photos = await window.uploadManager.restoreQueue(window.currentSession.id);
            if (photos && photos.length > 0) {
                console.log(`♻️ Restored ${photos.length} photos from local storage`);

                // Restore capturedPhotos array
                // We need to convert stored blobs back to object URLs for display
                capturedPhotos = photos.map(p => {
                    // Re-create object URL from blob if it exists
                    if (p.blob && !p.objectUrl) {
                        p.objectUrl = URL.createObjectURL(p.blob);
                    }
                    return p;
                });

                // Update UI counters if in capture state
                const pc = document.getElementById('photo-count');
                if (pc) pc.textContent = capturedPhotos.length;

                // If we have enough photos, we might want to offer to go to selection
                const totalPhotos = settings.total_photos || 3;
                if (capturedPhotos.length >= totalPhotos) {
                    console.log('♻️ Session appears complete, offering selection state...');
                    // Optional: Auto-redirect to selection if complete?
                    // For now, let's just ensure data is ready
                }
            }
        } catch (e) {
        ...
        await new Promise((resolve) => {
            ...
            const timer = setInterval(() => {
                ...
                if (current <= 0) {
                    ...
                    // Add a small pulse animation to the number
    ...
    function toggleCameraSettings() {
        console.log('📸 toggleCameraSettings called!');
        const panel = document.getElementById('camera-settings-panel');
        if (panel) {
            console.log('📸 Camera settings panel found, toggling visibility');
            panel.classList.toggle('hidden');
        } else {
            console.error('📸 Camera settings panel not found!');
    ...
    function switchCamera() {
        ...
        navigator.mediaDevices.enumerateDevices().then(devices => {
            ...
            if (isLogitechBrio && !IS_DEV) {
                console.log('🎥 LOGITECH BRIO DETECTED in switchCamera! Menggunakan konfigurasi khusus Logitech');
                // Override constraints for Logitech BRIO - more aggressive
                cameraConstraints.width = { min: 1920, ideal: 1920 };
                cameraConstraints.height = { min: 1080, ideal: 1080 };
            }

            console.log('⚙️ Switch camera constraints:', JSON.stringify(cameraConstraints, null, 2));
            ...
            navigator.mediaDevices.getUserMedia({ video: cameraConstraints })
                .then(stream => {
                    ...
                    if (videoTrack) {
                        const settings = videoTrack.getSettings();
                        console.log('📐 Actual resolution after switch:', `${settings.width}x${settings.height}@${settings.frameRate}fps`);
                    }
                ...
                .catch(error => {
                    ...
                    if (error.name === 'OverconstrainedError') {
                        console.log('⚠️ Fallback ke HD dalam switchCamera');
                        ...
                        navigator.mediaDevices.getUserMedia({ video: fallbackConstraints })
                            .then(fallbackStream => {
                                ...
                                console.log('⚠️ Switched with fallback HD resolution');
                                alert('Kamera berhasil diganti dengan resolusi HD!');
                            })
                            .catch(fallbackError => {
                                console.error('❌ Even fallback switch failed:', fallbackError);
            ...
            console.error('❌ Could not enumerate devices in switchCamera:', error);
            alert(`Gagal beralih ke kamera. Error: ${error.message}`);
        });
    }
    ...
    function changeCamera() {
        console.log('📷 changeCamera() called - redirecting to switchCamera()');
    ...
    console.log('🔄 Camera functions exported to window:');
    ...
    document.addEventListener('DOMContentLoaded', () => {
        console.log('🔄 Document loaded, checking camera functions:');
